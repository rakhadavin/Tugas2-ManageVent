...
Asynchronous Programming : paradigma pemoregraman dimana operasi dapat dieksekusi secara parallel dan bersamaan tanpa menunggu oeprasi lain selesai (multi-thread), sehingga respon yang didapat bisa sekaligus ( mulitple responses)

Synchronic Programming : paradigma singl-thread dimana hanya 1 operasi yang dapat dieksekusi dalam 1 waktu, dengan demikian respon yang dihasilkan juga hanya 1 saja

# Dalam penerapan JavaScript dan AJAX, terdapat penerapan paradigma event-driven programming. Jelaskan maksud dari paradigma tersebut dan sebutkan salah satu contoh penerapannya pada tugas ini.

Event-driven progamming : sebuah metode dalam pemrograman dimana program dijalankan berdasarkan event/kejadian sebagai sinyal untuk menjalankan suatu program yang siap dieksekusi.

contoh nya dalam tugas ini : button yang kita buat semua nya memiliki event-driven sendiri, seperti <button onclick="functionA()">Tambah Item</button> dimana button tersebut memiliki atribute onClick sebagai arti ketika di "klik" dia akan berfungsi sebagai sinyal kepada fungsi yang tertaut (functionA()) sebagai tanda bahwa button tersebut sedang di eksekusi dan harus menjalankn fungsi yang tertaut.

# Jelaskan penerapan asynchronous programming pada AJAX.

Asynchronous JavaScript and XML (AJAX) adalah gabungan dari teknologi pengembangan aplikasi web yang membuat aplikasi web menjadi lebih responsif terhadap interaksi pengguna. Dengan menggunajkan AJAX memungkinkan aplikasi web untuk bertukar data dengan server secara async.

cara kerja :
XMLHttpRequest merupakan objek yang dibuat senbagai perantara dalam pertukaran data dari server (request) yang kemudian di simpan dan disalurklan ke web kita dan melkakukan perubahan pada halaman web kita tanpa melaakukan refresh secara manual (autorefresh) sehingga data yang ada di web kita merupakan data terbaru.

dalam tugas ini kita meunggukana fetch API untuk menerapkan async AJAX

# Pada PBP kali ini, penerapan AJAX dilakukan dengan menggunakan Fetch API daripada library jQuery. Bandingkanlah kedua teknologi tersebut dan tuliskan pendapat kamu teknologi manakah yang lebih baik untuk digunakan.

Fetch API lebih mumpuni daripada library jQuery. Karena fleksibilitas karena menyembunyikan kompleksitas XMLResponse dan HTTPResponse sehingga kita hanya menggunakan methode methode yang tersedia saja untuk mengoperasikanobjek, seperti .length, .remove, .append, dsb. Kemudian Fetch API dapat menjalankan program(request) secara async sehingga tindakan yang dikerjakan dapat berbentuk parallel dan lebih responsif. Namun kelemahan dari Fetch API adalah ia akan tetap berjalan normal meski reponse yang dihasilkan error 404 atau 500, sehingga untuk kasus ini ketika menggunaan Fetch API kita harus handle sendiri.

Selain itu fetch API juga mendukung CORS (Cross Origin Resource Sharing) sehingga hal ini memungkinkan server untuk berbagi resource dengan origin ( domain,scheme, ataupun port) selain dirinya sendiri

Sehingga untuk saat ini saya memilih Fetch API untuk digunakan, selain karena syntax nya lebih ringkas, fleksibilitas dan kemudahan dalam menggunakannya jauh lebih baik dibandingkan JQuery Lib.

# Step-step menyelasikan tugas 6

selain melakukan apa yang dicontohkan di tutorial seperti membuat tabel melalui function, saya juga melaukan beberapa modifikasi :

1. membuat modal yang kemudian disertakan css animation, dan melakukan design modal popup untuk membuat barang baru sehingga nyaman digunakan user.

Modal yang saya buat merupakan modal yang dibuat manual bukan dari bootstrap melainkan pure HTML

2. Saya memiliki 2 fungsi tambah item, yang tersedia di Navbar merupakan fungsi yang lama (pindah homepage) yang dibawah berupa button adalah penerapan ajax

3. melakukan penambahan-pengurangan-penghapusan item debgan penerapan fetch API sehingga ketika dilakukan perubahan tidak perlu lagi dilakukan refresh manual (ada auto refresh function)
